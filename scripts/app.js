(function () {
    ...
    document.getElementById('butAddCity').addEventListener('click', function () {
        ...
        var data = active.transaction(["timeTables"], "readwrite");
        var object = data.objectStore("timeTables");
        var rgetAll=object.add({key: key, label: label});
 ...
 var indexedDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB;
            ...
            function startDB() {
                ...
                dataBase = indexedDB.open("timeTables", 2);
                dataBase.onupgradeneeded = function (e) {

                    var active = e.target.result;
                    var object = active.createObjectStore("timeTables", { keyPath : 'key'});
                };
            ...
            function add() {
                            ...
                            var data = active.transaction(["timeTables"], "readwrite");
                            var object = data.objectStore("timeTables");
                            var rgetAll=object.getAll()
                            rgetAll.onsuccess = function(event) {
                              app.selectedTimetables=rgetAll.result;
                              if(app.selectedTimetables.length>0)
                              {
                                app.selectedTimetables.forEach(function(tm) {
                                  app.getSchedule(tm.key, tm.label);});
                              }else{
